...
### Operation
Java app that uses websockets to synchronize text 'documents' with clients. Documents are grouped in 'classrooms', each on of which having a 'master' document. Student clients can view the master document and write their own documents, teacher clients can view all student documents and edit the master document. Classrooms are saved to disk periodically and all saved classrooms are loaded from disk on application start-up.
